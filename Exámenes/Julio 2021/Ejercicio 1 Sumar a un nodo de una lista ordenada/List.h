...
public:
	...
	// Implementa a continuación el método pedido y justifica su coste.
	//@ <answer>


	//Comenta el coste

	void attach(Nodo* node, Nodo* after) {
		node->_ant = after->_ant;
		node->_sig = after;
		after->_ant->_sig = node;
		after->_ant = node;
	}

	void dettach(Nodo* node) {
		node->_ant->_sig = node->_sig;
		node->_sig->_ant = node->_ant;
		node->_sig = nullptr;
		node->_ant = nullptr;
	}

	void add_to(const int pos, const int m) {
		Nodo* current = this->_prim;
		int i = 0;

		while (current != nullptr && i != pos) {		// Nos situamos en la posicion donde esta el nodo que buscamos.
			current = current->_sig;
			i++;
		}

		if (current != nullptr) {		// Sumamos el valor al nodo buscado.
			current->_elem += m;
		}

		Nodo* anterior = current->_ant;		// Guardamos el nodo anterior al buscado.
		Nodo* siguiente = current->_sig;	// Guardamos el nodo siguiente al buscado.

		while (siguiente != nullptr && current->_elem > siguiente->_elem) {		// Recorremos la lista hasta encontrar la posicion donde lo tenemos que colocar.
			if (anterior != nullptr) {		// Apuntamos el siguiente de anterior a siguiente.
				anterior->_sig = siguiente;
			}
			else {		// Si llegamos al final, siguiente es la head.
				_prim = siguiente;
			}
			siguiente->_ant = anterior;		// Apuntamos el anterior de siguiente a anterior.
			anterior = siguiente;			// Avanzamos anterior.
			siguiente = siguiente->_sig;	// Avanzamos siguiente.
		}

		// Colocamos el nodo en la posicion buscada.
		anterior->_sig = current;		// Primero asignamos el nodo anterior, apuntando del anterior al que vamos a introducir
		current->_ant = anterior;		// y del que vamos a introducir al anterior.
		current->_sig = siguiente;		// Después asignamos cuál es el siguiente del nodo que vamos a introducir
		if (siguiente != nullptr) {		
			siguiente->_ant = current;	// y apuntamos el nodo siguiente al que vamos a introducir.	
		}
		else {		// Si siguiente es nullptr eso es porque vamos a colocar el nodo en el último elemento.
			_ult = current;		
		}
	}
...
template <class T>
inline std::ostream& operator<<(std::ostream& o, List<T> const& a) {
	a.print(o);
	return o;
}

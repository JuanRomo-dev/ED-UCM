...
public:
	...
	void aLaCola(Queue<T>& colegas, const T& pringao) {
		Nodo* current = this->prim;
		Nodo* primC = colegas.prim;
		Nodo* resto = nullptr;			// Para guardar al resto de la cola

		while (current->sig != nullptr) {
			if (current->elem == pringao) {
				resto = current->sig;			// Guardamos el resto
				current->sig = primC;			// Colamos a los colegas detrás del pringao
			}
			current = current->sig;				// Avanzamos el nodo
		}
		
		if (resto != nullptr) {
			current->sig = resto;			// Colocamos al resto detrás de los colegas.
			colegas.prim = nullptr;
		}
		else if(current->elem == pringao) {		// Si el pringao es el último
			current->sig = primC;			// Se añaden los demás detrás
			colegas.prim = nullptr;
		}
	}

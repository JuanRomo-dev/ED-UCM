...
public:
	...
	void invertir() {
		if (this->numElems > 1) {
			Nodo* current = this->prim;			// Avanzamos current hasta situarnos en el ultimo elemento.
			Nodo* next = current->sig;
			Nodo* prev = nullptr;		// Se van metiendo en orden inverso por prev.
			...
			current->sig = prev;	// Colocamos el ultimo elemento el primero de la lista para terminar de invertirlo.
			prim = current;
		}
	}

...
public:
	...
	void dettach(Nodo*& node) {
		...
		else if (node->_sig != nullptr) {
			node->_ant->_sig = node->_sig;
			node->_sig->_ant = node->_ant;
			node->_sig = nullptr;
			node->_ant = nullptr;
			node = this->_prim->_sig;
		}
		else {
			node->_ant->_sig = nullptr;
			_ult = node->_ant;
			node = _ult->_sig;
		}
	}
	...
	/** interseccion **/
	//Explica el coste
	// El coste es lineal según el número de elementos de la lista 1. O(n). n = this->size().

	void interseccion(List<T> const& other) {
		// rellenar
		Nodo* current = this->_prim;		// Nodo para la lista 1.
		Nodo* cur = other._prim;			// Nodo para la lista 2 (other).

		while (current != nullptr) {
			if (current->_elem < cur->_elem) {
				dettach(current);
			}
			else if (current->_elem > cur->_elem) {
				cur = cur->_sig;
			}
			else if (current->_elem == cur->_elem) {
